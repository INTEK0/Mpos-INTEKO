...
namespace WindowsFormsApp2.Helpers
{
    public static class FormHelpers
    {
        ...
        public static IpModel GetIpModel()
        {
            try
            {
                string ip = null, model = null, merchantId = null;
                using (SqlConnection connection = new SqlConnection(DbHelpers.DbConnectionString))
                {
                    ...
                    string query = $@"select model = case when kf.KASSA_FIRMALAR=N'SUNMI' then 1 when kf.KASSA_FIRMALAR=N'AzSMART' then 2 when kf.KASSA_FIRMALAR=N'OMNITECH' then 3 when kf.KASSA_FIRMALAR=N'DATAPAY'
then 5 when kf.KASSA_FIRMALAR=N'NBA'then 6 when kf.KASSA_FIRMALAR=N'EKASAM' then 7 when kf.KASSA_FIRMALAR=N'XPRINTER' then 4 else  0 end , ip_ = case 
